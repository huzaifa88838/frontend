...
# Initialize user service and logger
...
def login():
    ...
    try:
        ...
        user, error = user_service.authenticate(data['username'], data['password'])
...
def get_users():
    ...
    # Otherwise, get users by parent (users they created)
        ...
        if parent_id and current_user.has_role_permission('supermaster'):
            users = user_service.get_users_by_parent(parent_id, skip, limit)
        else:
            # Default: get users created by current user
            users = user_service.get_users_by_parent(str(current_user._id), skip, limit)
    else:
        return jsonify({
            'success': False,
...
def get_user(user_id):
    ...
    if include_hierarchy and current_user.has_role_permission('master'):
        hierarchy = user_service.get_user_hierarchy(user_id)
...
def get_wallet(user_id):
    ...
    transactions = user_service.get_user_transactions(
...
def update_wallet(user_id):
    ...
    success, result = user_service.update_wallet_balance(
        user_id=user_id,
        amount=data['amount'],
...
def get_dashboard_stats():
    ...
    # Count users by role
    role_counts = user_service.count_users_by_role()
    
    # Get recent users
    recent_users = user_service.get_users_by_role('user', 0, 5)
